...
def p_expr_include(p):
    'expr : INCLUDE expr'
    p[0] = ast.Include(p[2], False, None, lineno=p.lineno(1))

def p_expr_include_once(p):
    'expr : INCLUDE_ONCE expr'
    p[0] = ast.Include(p[2], True, None, lineno=p.lineno(1))

def p_expr_require(p):
    'expr : REQUIRE expr'
    p[0] = ast.Require(p[2], False, None, lineno=p.lineno(1))

def p_expr_require_once(p):
    'expr : REQUIRE_ONCE expr'
    p[0] = ast.Require(p[2], True, None, lineno=p.lineno(1))
...
def p_error(t):
    if t:
         raise Exception('invalid syntax' +  str(t.lineno))
    else:
        #yacc.restart()
