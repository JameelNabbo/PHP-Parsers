class Node(object):
    ...
    def accept(self, visitor, recurse_depth=0):
        visitor.visit(self)
        if recurse_depth > 0:
            for field in self.fields:
...
Try = node('Try', ['nodes', 'catches', 'finally_'])
...
Closure = node('Closure', ['params', 'vars', 'nodes', 'static', "is_ref"])
...
ClassConstantAccess = node("ClassConstantAccess", ['class_', 'name'])
...
Eval = node('Eval', ['expr'])
Include = node('Include', ['expr', 'once', 'body'])
Require = node('Require', ['expr', 'once', 'body'])
...
UseDeclarations = node('UseDeclarations', ['type', 'nodes'])
...
# Newly Added nodes
GoTo = node("GoTo", ["node"])
NamespaceChain = node("NamespaceChain", ["nodes"])
NamespaceNameTail = node("NamespaceNameTail", ["name", "alias"])
NamespaceNameTailList = node("NamespaceNameTailList", ["nodes"])

# Placeholder Nodes
SquareCurly = node("TraitModifier", ["type", "expr"])
LambdaUse = node("LambdaUse", ["nodes"])
